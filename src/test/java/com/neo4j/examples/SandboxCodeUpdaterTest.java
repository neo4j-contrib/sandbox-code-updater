...
class SandboxCodeUpdaterTest {

    @Test
    void updates_Python_example(@TempDir Path tempDir) throws Exception {
        String repositoryName = "northwind";
        Path sourceCodePath = testResourcesPath("/fake-template-repo");

        new SandboxCodeUpdater(
                new FakeNorthwindRepositoryCloner(tempDir),
                new SandboxMetadataReader(),
                sourceCodePath
        ).update(repositoryName, tempDir);

        Path northwindCloneLocation = tempDir.resolve("northwind");
        Path pythonExample = northwindCloneLocation.resolve("code").resolve("python").resolve("example.py");
    ...
    @Test
    void updates_Java_example(@TempDir Path tempDir) throws Exception {
        String repositoryName = "northwind";
        Path sourceCodePath = testResourcesPath("/fake-template-repo");
        ...
        Path northwindCloneLocation = tempDir.resolve("northwind");
        Path javaExample = northwindCloneLocation.resolve("code").resolve("java").resolve("Example.java");
    ...
    @Test
    void updates_Csharp_example(@TempDir Path tempDir) throws Exception {
        String repositoryName = "northwind";
        Path sourceCodePath = testResourcesPath("/fake-template-repo");
        ...
        Path northwindCloneLocation = tempDir.resolve("northwind");
        Path javaExample = northwindCloneLocation.resolve("code").resolve("csharp").resolve("Example.cs");
        assertThat(String.join("\n", Files.readAllLines(javaExample))).isEqualTo(
